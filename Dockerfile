...
# Optional: install JS deps if present
COPY package.json yarn.lock ./
RUN yarn install --frozen-lockfile || true

# ---- Install JS deps (do this BEFORE copying the whole app for better caching) ----
COPY package.json yarn.lock ./
...
# (js/css bundling will run during assets:precompile if configured)
